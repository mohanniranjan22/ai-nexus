...
export function AppSidebar() {
  ...
  // ✅ FIXED version with null-safety
  const GetLastUserMessageFromChat = (chat) => {
    ...
    const lastUserMsg =
      userMessages.length > 0 ? userMessages[userMessages.length - 1].content : "No user messages";
      ...
      <SidebarContent>
        <SidebarGroup>
          <div className="p-3">
            ...
            {chatHistory?.map((chat, index) => {
              const lastChat = GetLastUserMessageFromChat(chat);
              return (
                <Link href={"?chat=" + chat.chatId} key={index} className="mt-2">
                  <div className="hover:bg-gray-100 p-3 cursor-pointer">
                    <h2 className="text-lg line-clamp-1">
                    ...
                    <p className="text-xs text-gray-400">{lastChat.lastMsgDate}</p>
                    <hr className="my-3" />
                  </div>
                </Link>
              );
